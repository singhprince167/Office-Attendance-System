    ...
    <div className="p-6 space-y-6">
      ...
      {allRecords.map(r => (
        <div key={r._id} className="p-4 border rounded bg-gray-50">
          <p><strong>{r.userId.name}</strong> ({r.userId.email})</p>
          <p>Date: {r.date}</p>
          ...
          {r.entryLocation && (
            <p>
              Entry Location: <a href={`https://www.google.com/maps?q=${r.entryLocation.lat},${r.entryLocation.lng}`} target="_blank" className="text-blue-600 underline">View Map</a>
            </p>
          )}
          {r.exitLocation && (
            <p>
              Exit Location: <a href={`https://www.google.com/maps?q=${r.exitLocation.lat},${r.exitLocation.lng}`} target="_blank" className="text-blue-600 underline">View Map</a>
            </p>
          )}
        </div>
      ))}
      ...
      {missingExit.length === 0 ? <p>All users have marked exit.</p> : missingExit.map(r => (
